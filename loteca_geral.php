...
function captura_proxima_rodada(){ // OK
	...
	$result=query($sql, 'loteca_geral(captura_proxima_rodada)');
	if($result==FALSE){
		return NULL;
	}else{
...
function captura_jogos_rodada($rodada){ // ok 
	...
	$result=query($sql, 'loteca_geral.php(captura_jogos_rodada)');
	if($result==FALSE){
		return NULL;
	}else{
		while ($row = mysqli_fetch_assoc($result)) {
...
function query($sql,$origem=''){
	global $mysql_link;
	if(!conecta()){
		$result=FALSE;
		grava_err_sql($origem . " : " . $sql);
	}else{
